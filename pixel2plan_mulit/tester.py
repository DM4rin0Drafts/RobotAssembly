...
def test_actor_critic(tangram):
    ...
    actor = Actor(n_features_actor, n_layers, embed_dim, n_heads, n_hidden_readout, dim_single_pred_actor)
    # Action is added to state, therefore the number of features is increased by 3 * n_token
    critic = Critic(n_features_critic, n_layers, embed_dim, n_heads, n_hidden_readout, dim_single_pred_critic)

    state_adj = tangram.state
    pred = actor(state_adj)
    actions = torch.zeros(pred.size())
    actions[tangram.control_nodes] = pred[tangram.control_nodes]
    values = critic(state_adj, actions)


def main():
    tokens = [Token(name=t) for t in ["square", "triangle1"]]  # , "square", "triangle1"]]
    tangram = Tangram(tokens=tokens, gui=True)
    test_actor_critic(tangram)
    time.sleep(1000)
