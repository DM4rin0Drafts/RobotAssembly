...
class Token(object):
    ...
    @property
    def path(self):
        path = os.path.join(os.getcwd(), f"environment/urdf/{self.name}.urdf")
        if not os.path.exists(path):
            raise RuntimeError(f'File {path} does not exist.')
        return path

    def _load_urdf(self):
        # TODO add scale factor for load urdf, tokens in simulation to big when multiple token spawnd
    ...
    def move(self, rel_vx: float, rel_vy: float, rel_wz: float):
        ...
        vx = STATE_SPACE_SIZE * rel_vx
        vy = STATE_SPACE_SIZE * rel_vy
        wz = np.pi * rel_wz
        ...
        self.center = self.center  # clip center position if action moves token outside of state space
    ...
    def hide(self):
        self.client.resetBasePositionAndOrientation(bodyUniqueId=self.ID,
                                                    posObj=[HIDDEN_POSITION, HIDDEN_POSITION, 0],
                                                    ornObj=(0, 0, 0, 0))
    ...
    def corners(self):
        ...
        Args:
            idx: Index of token in tangram tokenlist.
        ...
        for corner in corners:
            ...
            corner_scaled = corner * scaling        # TODO add scaling factor for corner, when multiple object spawned , this is a different scaling factor
